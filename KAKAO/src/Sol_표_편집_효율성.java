	...
	public static String solution(int n, int k, String[] cmd) {
			...
			for(int i=0;i<cmd.length;i++) {
				...
				case 'C':
					...
					int upIdx = node.get(selectedIdx).front;
					int downIdx = node.get(selectedIdx).back;
					if(upIdx!=-1)node.set(upIdx,new Node(node.get(upIdx).front,downIdx));
					if(downIdx!=n)node.set(downIdx,new Node(upIdx,node.get(downIdx).back));
				...
				case 'Z':
					...
					delCheck[zIdx]=false;
					node.set(zIdx,new Node(frontIdx,backIdx));
					
					if(frontIdx!=-1)node.set(frontIdx,new Node(node.get(frontIdx).front,zIdx));
					if(backIdx!=n)node.set(backIdx,new Node(zIdx,node.get(backIdx).back));
